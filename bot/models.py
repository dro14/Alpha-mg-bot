...
class TelegramUser(BaseModel):
    ...
    first_name = models.CharField(
        "First Name", default="", max_length=64, null=True, blank=True
    )
    last_name = models.CharField(
        "Last Name", default="", max_length=64, null=True, blank=True
    )

    def __str__(self):
        """STR Representation"""
        return (
            f"{self.user_id} - @{self.username} - {self.first_name}  {self.last_name}"
        )
...
class TelegramUserHistory(BaseModel):
    ...
    data_received = models.TextField(
        "Data received", max_length=4096, null=True, blank=True
    )
